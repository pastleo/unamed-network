...
async function main() {
  ...
  document.getElementById('join').addEventListener('click', async () => {
    ...
    const makePrimaryInput = document.getElementById('make-primary');
    input.style = 'border: yellow solid 1px;'

    const room = await unamedNetwork.join(input.value, makePrimaryInput.checked);
    input.style = room.members.size > 0 ? 'border: green solid 1px;' : 'border: blue solid 1px;';
  ...
  async function leaveRoom(roomName) {
    await unamedNetwork.leave(roomName);
    refreshRoomPeersTable();
  }
    ...
    unamedNetwork.rooms.forEach((room, roomNameHash) => {
      ...
      if (room.joined) {
        if (unamedNetwork.primaryRoomNameHash === roomNameHash) {
          ...
          roomTd.appendChild(document.createElement('br'));
          roomTd.appendChild(leaveBtn);
        }
      }
