...
class UnamedNetwork extends EventEmitter {
  ...
  removeRoomMember(room, member) {
    ...
    if (this.getRoomMembers(room).length > 0) return;
    ...
    if (room.joined && roomMemberState === 'joined') {
      this.emit('member-left', { member: peer, room });
    }
  ...
  async subscribePeerIpfsPubsub(peerId) {
    ...
    const pubsubs = await this.ipfs.pubsub.ls();
    if (pubsubs.indexOf(pubsubTopic) !== -1) return;
  ...
  handleJoinPacket(from, packet) {
    ...
    this.setRoomMembers(room, [from], 'joined');
  ...
  gotPingPongPayload(peerId, packet) {
    ...
    if (peer.nodeType === 'serviceNode' && packet.providing) {
      ...
      if (this.nodeType === 'clientNode' && packet.providing.gateway) {
