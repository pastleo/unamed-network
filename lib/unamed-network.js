...
class UnamedNetwork extends EventEmitter {
  ...
  async sendPacket(peer, packet) {
    ...
    if ([this.nodeType, peer.nodeType].indexOf('serviceNode') !== -1) {
      ...
      await this.sendRtcPacket(peer.rtc, packet);
    } else {
      throw new Error('sendPacket: peer is not serviceNode nor connected via rtc');
    }
  }

  sendRtcPacket(rtc, packet) {
    return new Promise(resolve => {
      rtc.sendingChunks.push(...packetToChunks(packet));
      rtc.sendingResolves.push(resolve);

      if (rtc.chunksSender) return;
      rtc.chunksSender = () => {
        const chunk = rtc.sendingChunks.shift();
        rtc.simplePeer.write(chunk);

        if (isEOF(chunk)) {
          rtc.sendingResolves.shift()();
        }
        if (rtc.sendingChunks.length <= 0) {
          delete rtc.chunksSender;
        } else {
          setTimeout(rtc.chunksSender);
        }
      }
      rtc.chunksSender();
    });
