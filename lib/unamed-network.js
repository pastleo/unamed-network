...
class UnamedNetwork extends EventEmitter {
  ...
  setPeer(peerId, payload) {
    ...
    if (peer.state === 'connected') {
      ...
      this.loopPeerKeepAlivePing(peer);
    }
  ...
  loopPeerKeepAlivePing(peer) {
    if (peer.keepAlivePingTimer) return;
    if (peer.keepAlivePingTimer === undefined) {
      logger.keepAlive(`keepAlivePing loop for '${peer.peerId}' starting with interval: ${KEEP_ALIVE_PING_INTERVAL}ms`);
    }
    peer.keepAlivePingTimer = setTimeout(async () => {
      const keepNext = await this.peerKeepAlivePing(peer);
      if (keepNext) {
        peer.keepAlivePingTimer = null;
        this.loopPeerKeepAlivePing(peer);
      } else {
        delete peer.keepAlivePingTimer;
      }
    }, KEEP_ALIVE_PING_INTERVAL);
  }

  async peerKeepAlivePing(peer) {
    const startedAt = Date.now();
    if (peer.state !== 'connected') {
      logger.keepAlive(`peer.state = ${peer.state}, stop pinging '${peer.peerId}] at ${new Date(startedAt)}`);
      return false;
    }
    logger.keepAlive(`starting pinging '${peer.peerId}' at ${new Date(startedAt)}`);

    try {
      await this.ping(peer.peerId);
      const took = Date.now() - startedAt;
      logger.keepAlive(`done pinging '${peer.peerId}', took ${took}ms`);
      return true;
    } catch (error) {
      console.warn(`keepAlivePing: error while pinging '${peer.peerId}', disconnecting...`, error);
