...
class Client {
  ...
  _registerGroupRequest(term, callbackFirst, callbackSecond, registeredSettings = {}) {
    ...
    const requestNextsRace = (group, msgId, fromAddr = '') => (payload = {}, reqSettings = {}) => (
      new Promise(resolve => {
        const settings = { ...this._config.defaultRequestSettings, ...registeredSettings, ...reqSettings };
  ...
  _sendLeave(addr, group) {
    try {
      this._connManager.send(addr, 'leave-group', { group });
    } catch (error) {
      console.warn(`send leave ${group} message to ${addr} error`, error);
    }
  }
  ...
  async _joinViaAddr(group, payload, addr, viaAddr) {
    ...
    if (this._connectingCheck(addr)) {
      ...
      try {
        ok = (await request(this._connManager, addr, 'join-group', { group, payload, viaAddr }, this._config.defaultRequestSettings.timeout, this._config.defaultRequestSettings.aliveTimeout)).ok;
      } catch (error) {
  ...
  async _maintainGroup(group) {
    ...
    if (this._hasJoined(group)) {
      nextRound = await this._ensureGroupInfo(group);
      if (nextRound) { return [nextRound, nextRejoin]; }

      const neighbors = this._getNeighbors(group);
      if (group === '/' && this._joinedGroups['/'].length === 0) {
        const rootMemberConnected = await this._doJoin('/');
        nextRound = rootMemberConnected;
        nextRejoin = rootMemberConnected;
      } else if (this._groupRejoining[group] && group !== '/') {
  ...
  async _ensureGroupInfo(group) {
    ...
    if (!this._groupInfo[group]) {
      const addr = neighbors[Math.floor(neighbors.length * Math.random())];
      try {
        this._groupInfo[group] = await request(this._connManager, addr, 'group-info', { group }, this._config.defaultRequestSettings.timeout, this._config.defaultRequestSettings.aliveTimeout);
      } catch (error) {
        console.warn(`request group-info of ${group} error`, { addr, error });
        return true;
      }
    }
    return false;
  ...
  _tooManyNeighbor(group) {
    ...
    this._sendLeave(neighborToSayBye, group);
    ...
    this._getNeighbors(group, excludedAddrs)
      .forEach(addr => {
        try {
          ...
          console.warn(`send broadcast ${term} message to ${addr} error`, { group, msgId, error });
        }
      });
  }
}
